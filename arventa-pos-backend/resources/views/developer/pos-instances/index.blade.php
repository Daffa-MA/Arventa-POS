                                    ...
                                    <td class="px-5 py-4 align-top">
                                        <div class="flex flex-wrap items-center gap-2" x-data="{ open: false, deploying: false }">
                                            ...
                                            <form method="post" action="{{ route('developer.pos.destroy', $instance) }}" onsubmit="return confirm('Hapus permanen POS {{ addslashes($instance->store_name) }} beserta data tenant terkait? Aksi ini tidak bisa dibatalkan.');">
                                                @csrf
                                                @method('delete')
                                                <button class="inline-flex items-center gap-1.5 rounded-lg border border-red-200 bg-red-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-red-700 active:scale-[0.97]">
