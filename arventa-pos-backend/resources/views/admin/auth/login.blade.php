<x-layouts.admin :title="($setting->admin_brand_name ?: 'Arventa POS').' Login'">
    <section class="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10">
        <div class="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-950/[0.04]">
            <div>
                <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Admin POS</p>
                <h1 class="mt-2 text-2xl font-semibold text-slate-950">{{ $setting->admin_brand_name ?: $setting->store_name }}</h1>
                <p class="mt-2 text-sm leading-6 text-slate-500">Masuk sebagai admin toko untuk mengelola katalog, setting, perangkat kasir, dan transaksi.</p>
            </div>
            ...
            <form method="post" action="{{ route('admin.login.store') }}" class="mt-6 grid gap-4">
                ...
                <button class="rounded-xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 active:scale-[0.98]">Masuk Admin</button>
            </form>
        </div>
    </section>
</x-layouts.admin>
